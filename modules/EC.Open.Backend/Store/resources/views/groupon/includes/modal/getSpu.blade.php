...
@section('footer')
    ...
    @include('store-backend::groupon.includes.modal.script')
    ...
    <script>
        ...
        function getList() {
            ...
            var postUrl = '{{route('admin.promotion.groupon.getSpuData')}}';
            ...
            $('.pages').pages({
                ...
                data.data.forEach(function (item) {
                    if (item.groupon_status == 1) {
                        item.hidden = 'hidden';
                        item.note = '该商品已参与其他拼团活动';
                    } else if(item.seckill_status == 1){
                        item.hidden = 'hidden';
                        item.note = '该商品已参与其他秒杀活动';
                    }
                    else {
                        item.hidden = '';
                        item.note = '';
