{!! Form::open( [ 'url' => [route('admin.promotion.seckill.updateDisable')], 'method' => 'POST', 'id' => 'base-form','class'=>'form-horizontal'] ) !!}
<input type="hidden" value="{{$seckill->id}}" name="id">
<div class="form-group">
    <label class="col-sm-2 control-label">活动名称：</label>
    <div class="col-sm-10">
        <input type="text" disabled value="{{$seckill->title}}" class="form-control" name="base[title]" placeholder=""
               required/>
    </div>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-10">
        <input type="text" class="form-control" value="{{$seckill->tags}}" disabled>
    </div>
</div>
            ...
            @foreach($items as $key=>$item)
                <tr data-key="{{$num+$key}}">
                    <td>
                        ...
                        <input type="hidden" name="item[{{$num+$key}}][item_id]" value="{{$item->item_id}}">
                    ...
                    <td><input type="text" disabled value="{{$item->seckill_price}}" class="form-control"
                               name="item[{{$num+$key}}][seckill_price]"></td>
                    ...
                    <td>
                        <a>
                            <i style="color: grey;"
                               class="fa  @if($item->get_point) fa-toggle-on @else fa-toggle-off @endif"
                               title="切换状态">
                    ...
                    <td>
                        <a>
                            <i style="color: grey;"
                               class="fa  @if($item->use_point) fa-toggle-on @else fa-toggle-off @endif"
                               title="切换状态">
                    ...
                    <td>
                        <a>
                            <i style="color: grey;"
                               class="fa switch @if($item->status) fa-toggle-on @else fa-toggle-off @endif"
                               title="切换状态">
                    ...
                    <td>
                        <input type="text" disabled value="{{$item->sell_num}}" class="form-control"
                               name="item[{{$num+$key}}][sell_num]">
                        <small>当前销量：{{$item->RealSale}}</small>
                    </td>
                    <td>
                        <input type="text" disabled value="{{$item->sort}}" class="form-control"
                    ...
                    <td>
                        ...
                        <a class="btn btn-xs btn-danger" disabled data-id="{{$item->item_id}}"
                           href="javascript:;">
