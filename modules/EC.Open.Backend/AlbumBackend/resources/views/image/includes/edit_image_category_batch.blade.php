...
@section('body')
    <div class="row">
        {!! Form::open( [ 'url' => [route('admin.image.saveBatch')], 'method' => 'POST', 'id' => 'base-form','class'=>'form-horizontal'] ) !!}
        <div class="form-group">
            <input value="{{$ids}}" type="hidden" name="ids">
            {!! Form::label('name','分类：', ['class' => 'col-sm-4 control-label']) !!}
            <div class="col-sm-8">
                <select class="form-control m-b " name="category_id">
                    @foreach($categories as $key => $val)

                        <option {{$val->id==$cid?'selected':''}} value="{{$val->id}}">{{ $val->html }} @if($val->parent_id !=0 )
